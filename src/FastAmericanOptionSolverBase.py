...
class AmericanOptionSolver(ABC):
    def __init__(self,
                 K: float,
                 r: float,
                 q: float,
                 vol: float,
                 l: int,  # quadrature_num?
                 n: int,  # collocation_num
                 m: int,  # Iterations for B(tau)
                 p: int,  # integration_num?
                 # T maturity,
    ...
    def solve(self, t, S0):
        if self.q == 0 and self.option_type == OptionType.Call:
            # for american call with no dividends, return european call price
            self.european_price = EuropeanOption(S0, self.K, self.T, self.r,
                                                 self.q, self.vol).call_value()
        ...
        v = self.american_value_with_known_boundary(tau, S0, self.r, self.q, self.vol, self.K)
        return v
    ...
    def test_numerical_integration(self):
        ...
        num_res = self.quadrature_sum(self.test_integrand, tau, S0, self.l)
        self.debug(
            "analytical sol = {0}, numerical sol = {1}, err = {2}".format(analy_res, num_res, abs(analy_res - num_res)))

    @staticmethod
    def test_integrand(tau, S, u, Bu):
        return S * u * np.exp(u * u)

    def american_value_with_known_boundary(self, tau, S0, r, q, vol, K):
        # input changes from tau to T to match w/ EuropeanOption
        if self.option_type == OptionType.Put:
            v = EuropeanOption(S0, K, tau, r, q, vol).put_value()
        else:
            v = EuropeanOption(S0, K, tau, r, q, vol).call_value()
        ...
        v12 = self.quadrature_sum(self.v_integrand_12, tau, S0, self.p)
        return v + v12
    ...
    def compute_integration_terms(self, tau, num_points):
        ...
        dqplus = DQPlus(self.K, self.r, self.q, self.vol, self.n, self.l, self.option_type)
        ...
        self.shared_u, self.shared_Bu = dqplus.run_full_algorithm(self.m)
    ...
    #     cheby_interp = ChebyshevInterpolation(H, self.to_cheby_point, self.tau_min, self.tau_max)
    #     # ci = ChebyshevInterpolation(self.n)
    #     # ci.compute_nodes(self.tau_min, self.tau_max)
    #     # cheby_interp = ci.get_nodes()
    #
    ...
    def set_initial_guess(self):
        ...
        qd = DQPlus(self.K, self.r, self.q, self.vol, self.n, self.l, self.option_type)
        res = []
        for tau_i in self.shared_tau:
            res.append(qd.run_full_algorithm(self.m))
        self.shared_B = res
    ...
    def check_value_match_condition1(self):
        ...
        for tau_i, B_i in zip(self.shared_tau, self.shared_B):
            ...
            right.append(self.american_value_with_known_boundary(tau_i, B_i, self.r, self.q, self.vol, self.K))
    ...
    def check_f_with_B(self, B=np.linspace(50, 150, 30)):
        if self.option_type == OptionType.Call and self.q == 0:
            return
        tau = 0.2
        fprime = []
        f = []
